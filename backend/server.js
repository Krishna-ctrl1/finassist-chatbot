...
app.get('/api/dashboard/data', authenticateToken, async (req, res) => {
  try {
    ...
    if (userData.orders && userData.orders.length > 0) {
      ...
      portfolioData.totalReturns = portfolioData.totalValue - portfolioData.totalInvested;
      portfolioData.returnPercentage = totalInvested > 0 ? ((portfolioData.totalReturns / totalInvested) * 100) : 0;
    ...
    if (userData.orders && userData.orders.length > 0) {
      const sipOrders = userData.orders.filter(order => 
        order.frequency && (order.payment_status === 'Paid' || order.payment_status === 'completed')
      );
      ...
      sipData.totalMonthlyInvestment = sipOrders.reduce((sum, order) => {
        ...
        } else if (order.frequency === 'quarterly' || order.frequency === 'QUARTERLY') {
          return sum + ((parseFloat(order.amount) || 0) / 3);
        } else if (order.frequency === 'yearly' || order.frequency === 'YEARLY') {
    ...
    if (userData.orders && userData.orders.length > 0) {
      ...
      recentOrders.forEach(order => {
        transactions.push({
          type: order.frequency ? `SIP - Investment ${order.id}` : `Lumpsum - Investment ${order.id}`,
          amount: parseFloat(order.amount) || 0,
    ...
    const goals = [
      {
        ...
        progress: Math.min((portfolioData.totalValue * 0.4 / 100000) * 100, 85)
      },
      {
        name: 'House Down Payment',
        target: 2000000,
        current: Math.min(portfolioData.totalValue * 0.6, 1200000),
        progress: Math.min((portfolioData.totalValue * 0.6 / 2000000) * 100, 60)
      }
    ];
